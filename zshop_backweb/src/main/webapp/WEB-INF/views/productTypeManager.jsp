...
<head>
    ...
    <script>
    	...
    	$(function(){
    	    //如果当前页面数据为空，则加载最后一页的数据,这里针对删除商品类型时可能出现的显示bug
    	    if(${pageInfo.pages<pageInfo.pageNum}){
                //重新加载数据
                location.href='${pageContext.request.contextPath}/back/productType/findAll?pageNum='+${pageInfo.pages};
            }
    	    $('#pagination').bootstrapPaginator({
	    			...
	    			{
	    				return '${pageContext.request.contextPath}/back/productType/findAll?pageNum='+page;
	    			},
    	...
    	function addProductType(){
    		$.post(
    			'${pageContext.request.contextPath}/back/productType/add',
    			...
    			function(result)
                {
                    ...
                    function () {
                        //重新加载数据
                        location.href='${pageContext.request.contextPath}/back/productType/findAll?pageNum='
                            +${pageInfo.pageNum};
                    });
    			}
    		);
        ...
        function showProductType(id){
    	    $.post(
    	        '${pageContext.request.contextPath}/back/productType/findById',
                {'id':id},
                function (result) {
                    if(result.status == 1)
                    {
        ...
        //展示删除提醒的modal框
        function showRemoveProductTypeModal(id) {
            $('#removeProductTypeModal').modal('show');
            $('#removeProductTypeId').val(id);
        }
        //删除商品类型
        function removeProductType() {
            $.get(
                '${pageContext.request.contextPath}/back/productType/removeById',
                {'id':$('#removeProductTypeId').val()},
                function (result) {
                    layer.msg(result.message,
                        {time:1000},
                        function () {
        ...
        function modifyStatus(id,btn) {
            $.get(
                '${pageContext.request.contextPath}/back/productType/modifyStatus',
                {'id':id},
                function (result) {
                    if(result.status = 1){
...
<body>
	                    	...
	                    	<tr>
	                            ...
	                            <td class="text-center">
	                                ...
	                                <input type="button" class="btn btn-warning btn-sm doProTypeDelete"
                                           onclick="showRemoveProductTypeModal(${productType.id})" value="删除">
    ...
    <!-- 删除商品类型 start -->
    <div class="modal fade" tabindex="-1" id="removeProductTypeModal">
        <!-- 窗口声明 -->
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <input type="hidden" id="removeProductTypeId"/>
                    <button class="btn btn-warning updateProType" onclick="removeProductType()" data-dismiss="modal">删除</button>
                    <button class="btn btn-primary cancel" data-dismiss="modal">取消</button>
                </div>
            </div>
        </div>
